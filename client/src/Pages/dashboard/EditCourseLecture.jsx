...
function EditCourseLecture() {
    ...
    const handleVideo = (e) => {
        ...
        if (video) {
            ...
            setData({ ...data, lecture: video, videoSrc: source });
        }
    };
    ...
    const handleSubmit = async (event) => {
        ...
        // Attach instructor/admin info if needed
        const payload = { ...data };
        // Assuming you have currentUser info in localStorage or Redux
        const user = JSON.parse(localStorage.getItem('user'));
        if (user) payload.updatedBy = { id: user._id, name: user.name };

        const res = await dispatch(updateLecture(payload));
